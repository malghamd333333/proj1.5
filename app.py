...
def search_venues():
    ...
    # user = db.session.query(User).filter_by(User.username.ilike("%ganye%")).all()
...
def create_venue_submission():
    ...
    flash('Venue ' + request.form['name'] + '|||' + name + city + request.form['address'] + ' was successfully listed2!')
...
def create_artist_submission():
    ...
    facebook_link = form.facebook_link.data
    genres = form.genres.data
    obj = Artist(name=name, city=city, state=state, phone=phone,
                 genres=genres, facebook_link=facebook_link)
    db.session.add(obj)
    db.session.commit()
